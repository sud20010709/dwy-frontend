...
export class ProductService {
  ...
  private categoryUrl = 'http://localhost:9090/api/cars';
  ...
  addProduct(product: Product,car: ProductCategory ): Observable<Object>{
  ...
  getProductCar(theProductId: number): Observable<ProductCategory> {

    // need to build URL based on product id
    const productUrl = `${this.baseUrl}/${theProductId}/car`;

    return this.httpClient.get<ProductCategory>(productUrl);
  }

  getProductSeller(theProductId: number): Observable<Product> {

    // need to build URL based on product id
    const productUrl = `${this.baseUrl}/${theProductId}/seller`;
  ...
  getProductListPaginate(thePage: number, 
    ...
    if(sortingMethod == "First Created"){
      ...
      this.getListURL = `${this.baseUrl}/search/findByCar?car=${theCategoryId}`
        + `&page=${thePage}&size=${thePageSize}`;
    }
    else if(sortingMethod == "Last Created"){
      this.getListURL = `${this.baseUrl}/search/findByCarOrderByDateCreatedDesc?car=${theCategoryId}`
      + `&page=${thePage}&size=${thePageSize}`;
    }

    else{
      this.getListURL = `${this.baseUrl}/search/findByCarOrderByNameAsc?car=${theCategoryId}`
      + `&page=${thePage}&size=${thePageSize}`;
    }
